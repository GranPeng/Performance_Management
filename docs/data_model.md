...
| 7 | Metric | 一个岗位的一项 KPI 定义（按规则版本） | 规则定义 |
| 8 | Target | 一个期间×一个指标或预算项目×一组维度的一个目标值 | 事实（长表） |
...
| Employee_ID | TEXT→Employee | 条件必填 | 个人粒度实际值；四类主体至少填一类 | 采集流程 | 个人指标计算 | 不变 |
...
| Target_Entity | TEXT | 是 | 校验对象（14 实体之一） | 定义方 | 校验流程 | 稳定 |
